...
class Ingredient(models.Model):
    ...
    quantity = models.FloatField(
        default=0
    )  # quantity available in the inventory (e.g., 4.5)
    unit = models.CharField(
        max_length=50, default="unitless"
    )  # unit of measurement (e.g., "tbsp", "lbs")
    unit_price = models.DecimalField(
        max_digits=10, decimal_places=2
    )  # price per unit (e.g., 0.05)
...
class MenuItem(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2
    )  # price of the menu item (e.g., 3.49)
...
class RecipeRequirement(models.Model):
    ...
    )  # reference to MenuItem table
    ingredient = models.ForeignKey(
        Ingredient, on_delete=models.CASCADE
    )  # reference to Ingredient table
    quantity_required = (
        models.FloatField()
    )  # quantity of ingredient needed for menu item (e.g., 1.5 ounces of "sugar" to create "Chocolate Cake")
